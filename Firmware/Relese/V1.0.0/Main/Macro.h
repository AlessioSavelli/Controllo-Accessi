...
//---Convert millis to format time
String _MY_MACRO_CONVERTION_TIME1(unsigned long time_to_convert) {
  unsigned long times = millis();
  int millisec = times % 100;
  int tseconds = times / 1000;
  int tminutes = tseconds / 60;
  int seconds = tseconds % 60;
  int thour = tminutes / 60;
  return "Time: " + String(thour, DEC) + ":" + String(tminutes, DEC) + ":" + String(seconds, DEC);
}

#define TIMEFORMAT_MILLIS _MY_MACRO_CONVERTION_TIME1(millis())
#define TIMEFORMAT_MILLIS_CHAR _MY_MACRO_CONVERTION_TIME1(millis()).c_str()

#define TIMEFORMAT_MYVALUE(val) _MY_MACRO_CONVERTION_TIME1(val)
#define TIMEFORMAT_MYVALUE_CHAR(val) _MY_MACRO_CONVERTION_TIME1(val).c_str()
